                ...
                <div className="space-y-3">
                  {shorts.map((s) => (
                    <Card key={s.id}><CardContent className="p-4 space-y-2">
                      ...
                      <textarea className="w-full border rounded p-2 text-sm" rows={3} onBlur={async (e) => { const value = e.target.value; setAnswers((x) => ({ ...x, [s.id]: { value } })); await submitAnswer(s.id, value, false); }} />
